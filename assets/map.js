...
Game.Map.prototype.open = function(x, y, z) {
    // If the tile is diggable, update it to a floor
    if (this.getTile(x, y, z).isClosedDoor()) {
        this._tiles[z][x][y] = Game.Tile.openDoorTile;
    }
};
...
Game.Map.prototype.swapPositions = function(
    entity, entity2) {
    var key1 = entity.getX() + ',' + entity.getY() + ',' + entity.getZ();
    var key2 = entity2.getX() + ',' + entity2.getY() + ',' + entity2.getZ();
    ...
    this._entities[key1] = entity2;
    this._entities[key2] = entity;
};
...
Game.Map.prototype.getContainersAt = function(x, y, z) {
    return this._containers[x + ',' + y + ',' + z];
};
...
Game.Map.prototype.setContainerItemsAt = function(x, y, z, items) {
	this._containers[x + ',' + y + ',' + z].items = items;
};
...
Game.Map.prototype.addContainer = function(x, y, z, container) {
	// If our items array is empty, then delete the key from the table.
    var key = x + ',' + y + ',' + z;
    var newContainer = {
		items: container.items || [],
		spawnList: container.spawnList || [],
		spawned: container.spawned || false,
		spawn: function() {
			for (var i = 0; i < this.spawnList.length; i++) {
				var spawn = this.spawnList[i];
				var spawnrate = 1 + (z/10);
				if (!this.spawned && spawn.chance && spawn.item && Math.random() < (spawn.chance*spawnrate)) {
					this.items.push(Game.ItemRepository.create(spawn.item));
				}
			}
			this.spawned = true;
		}
    }
    this._containers[key] = newContainer;
};
